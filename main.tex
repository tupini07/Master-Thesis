...
The evaluation results for both the baseline and ensemble classifiers presented in this section have been obtained by performing a 5-fold cross validation \cite{Kohavi:1995_study_of_cross_validation} procedure on the training data. 

The evaluation results for all models for each specific concrete entity can be found in \autoref{sec:apx-ensemble-eval-results}.

\section{Cross Validation}
\label{sec:cross-validation}

% maybe own section for explaining what cross validation is
...
\section{Evaluation results for each model and concrete entity}

This section presents the evaluation results obtained by each model when applied to a specific concrete entity. The first section shows the results of the baseline classifiers, while the second shows those for the ensemble classifiers. The data used to generate this results was the Wikidata training dumps and the external catalog dumps mentioned in \autoref{sec:orig-shape-of-data}.

\subsubsection{Baseline Evaluation Results}
\label{sec:apx-baseline-eval-results}
