...
\subsubsection{Similar Death Dates}
...
Voting classifier appears to have the best precision for \textit{Musicbrainz}, however this is also the model which has the worst \textit{recall} for that catalog. On the other hand, \textit{stack classifier}, which is the best F1 and recall, is the one with the worst precision. 
...
The best F1 score for Discogs is given to the \textit{stack classifier}. Like in IMDb, this best F1 model is actually does not have the best precision nor recall. Best precision is given by the \textit{gated classifier}, which is also the model with the worst recall and F1 score. \textit{Soft voting} classifier is the one which gives the best recall, and it also has the worst precision.

The table below shows a summary of the scores obtained by each model. These have been obtained by averaging all the model's scores. The rows are ordered by \textit{Avg. F1} in decreasing order.
...
From the results above it can be seem that both \textit{stack based} models have the best average precision. These are \textit{gated classifier}, which has the best precision, and \textit{stack classifier}, which has the second best. On the other hand, \textit{voting based} classifiers have the best recall: \textit{hard voting}, which has the best recall, and \textit{soft voting} which has the second best.

\textit{Soft voting} has the best precision when compared with \textit{hard voting}. Probably because soft voting considers an average of its base models predictions, so if a model is \textit{very} certain about a match then this amount will be taken into consideration for the final prediction. On the hand, \textit{hard} only considers how many base models predicted a positive or negative, and not how certain they were about this prediction. 

When it comes to the F1 score, the best performing model seems to be \textit{soft voting}. While \textit{stack classifier} is the worst. 

When it comes to precision for the \textit{voting} classifiers, \textit{soft voting} has the best among them

From the results above it can be seen that the \textit{stack classfier} seems to be the one with the highest precision, however tends to have a low recall. 


An important thing to note in the table above is that \textit{soft voting} is the classifier with the least standard deviation. This is important since it gives us more certainty that real world performance will be similar as that evaluated. ???explain why???

It seems that \textit{stack classifier} almost always has the worst recall
...
See STDs. Should be higher for ensembles ? or lower? or same? 

% a table which compares the top-3 models for each entity (entity vs [model, f1, f1std ...])
