...
\soweego, as record linkage, can be viewed as a series of step which are closely related with that of the record linkage framework presented in \autoref{chap:state-of-the-art}. In \autoref{chap:apprach}, an explanation about every part of \soweego will be given, together with what happens during each part. 
...
\todo[inline,color=green!25]{should do a \textit{contributions subsection?}. Introduce Wikidata section below }
...
Another thing that should be kept in mind is that the results presented for the ensemble models are for ensemble who use as their base classifiers all models available in the set of baseline classifier. This might not actually be optimal, and as said in the previous section, there may be some models which are driving the ensembles to make miss-predictions. A more careful construction of the base model set may improve the final performance of the ensembles. 



However, for the current project the use of ensembles is. good. not so good. soweego preference precision/recall?
