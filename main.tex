...
Besides this, the linker also applies a couple of \textit{post-classification} steps on the predictions. Specifically, what it does is to apply the following \textit{linking rules} to each result in turn.
...
This feature is extracted by taking the \textit{name} fields of both entities and checking if they're the same. This field may have more than one value (e.g., in the case of name variations), so the feature compares all possible pairs of names, and optimistically \textit{1} if any of the comparisons is a match. Remember that the values of features may go from \textit{0} (not a match), to \textit{1} (perfect match). 
...
It can be seen that some of the observations made for the results of the baseline models in the previous section still hold for these results as well. For instance, also for ensembles it seems that \textit{Musicbrainz/Musician} is the concrete entity that has the better \textit{F1 score}. This is to be expected since ensembles are just joining the results from their base classifiers. 

\textit{Discogs/Musician} seems to be the entity with the second best \textit{F1}.

For precision, all ensemble models also have \textit{Musicbrainz/Musician} as their best score. However, for recall, it seems that in all cases except for the \textit{gated classifier}, the highest score is for \textit{Discogs/Musician} (\textit{gated classifier} has \textit{Musicbrainz/Musician} as the top recall). 
