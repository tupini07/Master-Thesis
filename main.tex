...
In simple terms, probabilistic record linkage \cite{Sayers2015} compares a shared subset of fields of a given entity pair, then decides, whether the pair is a match or not, with a certainty value. The main difference concerning \textit{deterministic record linkage} is that now it is allowed for fields to partially match, and probabilistic record linkage considers a likelihood that the given attribute matches.
...
In its most basic form, two entities are taken and compared in all their fields; which may then match completely if they are exactly the same, or partially match if they are similar but not equal. This \textit{partial match} of the fields is measured by a percentage stating how similar they actually are. 

Any similarity metric can be used to measure partial similarity. For example, \textit{edit distance} for strings, which is basically the number of changes needed to transform one of the strings into the other to make them equal (either by adding, removing or updating characters). 
...
information about a person. However, this paper did not define any actual method
