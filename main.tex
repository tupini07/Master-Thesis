...
\end{tabular}
\caption{Best three models and their scores for each respective metric. These results are shown for each concrete entity.}
\label{tab:comparison-bl-ens-top-3}
\end{table}

It can be noticed that in almost all cases, the list of \textit{top 3} models for each metric, and concrete entity, includes both classifiers coming from the set of baseline classifiers and others which are ensembles. The exceptions for this are \textit{Precision} for \textit{IMDb/Actor}, where the list of top-3 is composed only of ensemble models. The \textit{Recall} for the same model is instead composed only of baseline models. The list for \textit{Recall}, for \textit{IMDb/Musician} is also composed of only baseline classifiers.

---
Graphic showing how many times each classifier appeared first, how many times second, and how many times third
---
comparative barplot plot of the scores for the top3 baseline and top3 ensemble
---
graphic of barplots with metrics for these top5
---
compare STDs. See if ensemles have higher or lower. What did we expect?
--- 
